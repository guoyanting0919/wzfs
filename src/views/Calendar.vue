<template>
  <div id="Calendar">
    ...
    <div class="filterBox">
      ...
      <el-button type="primary" class="adSearch" v-if="isLogin">進階搜尋</el-button>
    </div>
    ...
    <el-dialog custom-class="eventDailog" :visible.sync="eventDailog">
      ...
      <div class="dialogMain">
        ...
        <div class="joinUserBox" v-if="dialogEvent.JoinUsers">
          ...
          <el-table :data="dialogEvent.JoinUsers">
...
export default {
  ...
  computed: {
    ...
    //   if (vm.eventTypeData) {
    //     let typeArr = [];
    //     vm.eventTypeData.forEach(item => {
    //       !typeArr.includes(item.EventType)
    //         ? typeArr.push(item.EventType)
    //         : false;
    //     });
    //     return typeArr;
    //   }
  ...
  methods: {
    ...
    logInCheck() {
      ...
      console.log(check);
      check ? (this.isLogInG = true) : (this.isLogInG = false);
    },
    eventRender(info) {
      const vm = this;
